...
class ensemble(nn.Module):
    ...
    def forward(self, data):
        ...
        x = data.x.to(device)
        pro_emb = data.pro_emb.to(device)
        pro_emb_batch = data.pro_emb_batch.to(device)
        self.kcat = self.kcat.to(device)
        self.km = self.km.to(device)
        self.o = self.o.to(device)
        kcat_out = self.kcat(x, pro_emb, pro_emb_batch)
        km_out = self.km(x, pro_emb, pro_emb_batch)
        out = self.o(torch.cat([kcat_out, km_out], dim=1))
        return out
