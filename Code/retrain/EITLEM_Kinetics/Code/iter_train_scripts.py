...
def KKMTrainer(kcatPath, kmPath, TrainType, Iteration, log10, molType, device):
    ...
    for epoch in range(1, Epoch + 1):
        train_MAE, train_rmse, train_r2, loss_train, pcc_train = trainer.run(model, train_loader, optimizer, len(train_set), f"{Iteration}iter epoch {epoch} train:")
        MAE_dev, RMSE_dev, R2_dev, loss_dev, pcc_dev = tester.test(model, valid_loader, len(test_set), desc=f"{Iteration}iter epoch {epoch} valid:")
        scheduler.step()
        writer.add_scalars("loss",{'train_loss':loss_train, 'dev_loss':loss_dev}, epoch)
        writer.add_scalars("RMSE",{'train_RMSE':train_rmse, 'dev_RMSE':RMSE_dev}, epoch)
        writer.add_scalars("MAE" ,{'train_MAE' :train_MAE , 'dev_MAE' :MAE_dev }, epoch)
        writer.add_scalars("R2"  ,{'train_R2'  :train_r2  , 'dev_R2'  :R2_dev  }, epoch)
        writer.add_scalars("PCC" ,{'train_PCC' :pcc_train , 'dev_PCC' :pcc_dev }, epoch)
        tester.save_model(model, file_model+f'{molType}_trainR2:{train_r2:.4f}_devR2_{R2_dev:.4f}_RMSE_{RMSE_dev:.4f}_MAE_{MAE_dev:.4f}_PCC_{pcc_dev:.4f}') # 保存
