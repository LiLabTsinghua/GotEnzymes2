...
from KCM_CNNGNN import EitlemKcatPredictor
...
class EitlemDataSet(Dataset):
    def __init__(self, data, model_type, kinetic_parameter, log10=False):
        super(EitlemDataSet, self).__init__()
        self.data = data
        self.log10 = log10
        self.ngram = 3
        self.smi_model, self.seq_model = model_type
        
        # self.fingerprint_dict = load_input_from_pkl(f'../Data/CNNGNN/fingerprint_dict_{kinetic_parameter}.pickle')
        # self.word_dict = load_input_from_pkl(f'../Data/CNNGNN/sequence_dict_{kinetic_parameter}.pickle')
        
        if self.smi_model == 'gnn':
            self.atoms = load_input_from_pkl(f'../Data/CNNGNN/compounds_{kinetic_parameter}.pkl')
            self.adj = load_input_from_pkl(f'../Data/CNNGNN/adjacencies_{kinetic_parameter}.pkl')
        else:
            self.smiles_embedding = load_input_from_pkl(f'../../../pretrain/saved_models/{self.smi_model}.pkl')
        if self.seq_model == 'cnn':
            self.proteins = load_input_from_pkl(f'../Data/CNNGNN/proteins_{kinetic_parameter}.pkl')
        else:
            self.sequence_embedding = load_input_from_pkl(f'../../../pretrain/saved_models/{self.seq_model}_L.pkl')
    ...
    def __getitem__(self, idx):
        ...
        if self.smi_model == 'gnn':
            atoms = torch.LongTensor(self.atoms[smi_id])
            adj = torch.FloatTensor(self.adj[smi_id])
            smi_emb = (atoms, adj)
        else:
            smi_emb = torch.FloatTensor(self.smiles_embedding[smi_id])
        
        if self.seq_model == 'cnn':
            protein_emb = torch.LongTensor(self.proteins[pro_id])
        else:
            protein_emb = torch.FloatTensor(self.sequence_embedding[pro_id])
        
        value = math.log10(value) if self.log10 else math.log2(value)
        data = Data(x = torch.FloatTensor(smi_emb).unsqueeze(0), pro_emb=torch.FloatTensor(protein_emb), value=value, model = f'{self.smi_model}_{self.seq_model}')
        return data
...
def kineticsTrainer(kkmPath, TrainType, Type, Iteration, log10, molType, device, smi_model, seq_model, kinetic_parameter):
    ...
    dict2 = {'esm2': 1280, 'esm1b': 1280, 'esm3b': 2560, 'esm15b': 5120, 'prott5': 1024, 'prollama': 4096, 'cnn': 256}
    smi_shape = dict1[smi_model]
    seq_shape = dict2[seq_model]
    # dataset = EitlemDataSet(data, model_type=(smi_model, seq_model), log10=log10)
    ...
    train_set = EitlemDataSet(train_pair_info, model_type=(smi_model, seq_model), kinetic_parameter=kinetic_parameter, log10=log10)
    test_set = EitlemDataSet(test_pair_info, model_type=(smi_model, seq_model), kinetic_parameter=kinetic_parameter, log10=log10)
    ...
    if kkmPath is not None:
        ...
        if Type == 'KCAT':
            model = EitlemKcatPredictor(kinetic_parameter, smi_shape, 512, seq_shape, 10, 0.5, 10)
...
def KKMTrainer(kcatPath, kmPath, TrainType, Iteration, log10, molType, device, smi_model, seq_model):
    ...
    smi_shape = dict1[smi_model]
    seq_shape = dict2[seq_model]
    model = ensemble(kinetic_parameter, smi_shape, 512, seq_shape, 10, 0.5, 10)
...
def TransferLearing(Iterations, TrainType, log10=False, molType='MACCSKeys', device=None):
    ...
    smi_model_list = ['gnn']
    # seq_model_list = ['prott5', 'prollama', 'esm1b', 'esm2', 'esmc', 'cnn']
    seq_model_list = ['cnn']
