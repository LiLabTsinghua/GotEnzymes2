...
def kineticsTrainer(kkmPath, TrainType, Type, Iteration, log10, molType, device, smi_model, seq_model, kinetic_parameter):
    ...
    train_loader = EitlemDataLoader(data=train_set, batch_size=200, shuffle=True, drop_last=False, num_workers=4, prefetch_factor=10, persistent_workers=True, pin_memory=True)
    valid_loader = EitlemDataLoader(data=test_set, batch_size=200, drop_last=False, num_workers=4, prefetch_factor=10, persistent_workers=True, pin_memory=True)
    ...
    try:
        ...
        if os.path.exists(file_model):
            print(f"Deleting the folder: {file_model}")
            shutil.rmtree(file_model)
        raise
    finally:
        if 'writer' in locals():
            writer.close()
...
def KKMTrainer(kcatPath, kmPath, TrainType, Iteration, log10, molType, device, smi_model, seq_model):
    ...
    if os.path.exists(f'../Results/KKM/{train_info}'):
        print('MODEL KKM ALREADY EXISTS')
        return None
    ...
    kcat_pretrained = torch.load(kcatPath, weights_only=False)
    km_pretrained = torch.load(kmPath, weights_only=False)
    ...
    try:
        ...
        print(f"An error occurred: {e}")
        if os.path.exists(f'../Results/KKM/{train_info}'):
            print(f"Deleting the folder: ../Results/KKM/{train_info}")
            shutil.rmtree(f'../Results/KKM/{train_info}')
        raise
    finally:
        if 'writer' in locals():
            writer.close()
