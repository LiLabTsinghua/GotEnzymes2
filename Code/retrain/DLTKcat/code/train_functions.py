...
def scores(label, pred ):
    ...
    if np.isnan(label).any():
        print("Warning: Labels contain NaN")
        print(label)
    elif np.isinf(label).any():
        print("Warning: Labels contain Inf")
        print(label.shape)
        print(label)
    if np.isnan(pred).any():
        print("Warning: Predictions contain NaN")
        print(pred)
    elif np.isinf(pred).any():
        print("Warning: Predictions contain Inf")
        print(pred)
        # label = np.nan_to_num(label, nan=0.0, posinf=np.finfo(np.float64).max, neginf=np.finfo(np.float64).min)

    if np.isnan(pred).any() or np.isinf(pred).any():
        print("Warning: Predictions contain NaN or Inf values.")
        pred = np.nan_to_num(pred, nan=0.0, posinf=np.finfo(np.float64).max, neginf=np.finfo(np.float64).min)
