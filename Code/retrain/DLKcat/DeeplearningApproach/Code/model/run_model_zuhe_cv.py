...
class DynamicMLP(nn.Module):
    ...
    def _initialize_layers(self, input_dim):
        ...
        for _ in range(2):  # 前两个隐藏层
            current_dim = max(20, current_dim // 2)  # 确保维度不小于20
            hidden_dims.append(current_dim)
        
        # 构建三层网络
        self.layers.append(nn.Linear(input_dim, hidden_dims[0]))
        self.layers.append(nn.Linear(hidden_dims[0], hidden_dims[1]))
        self.layers.append(nn.Linear(hidden_dims[1], 20))  # 最终输出层
...
#             xs = F.leaky_relu(self.W_cnn[i](xs))
    ...
    def forward(self, inputs):
        ...
        seq_emb, smi_emb = inputs
        cat_vector = self.attention_cnn(seq_emb, smi_emb, layer_cnn)
...
if __name__ == "__main__":
    ...
    for cv in range(1, 5):
        ...
        data = pd.read_csv(f'../../Data/input/cv{cv}/EITLEM_KCAT.csv')
        seq_list = ['esm2', 'esm1b', 'esm3b', 'prott5', 'prollama']
        smi_list= ['maccskeys', 'smitrans', 'molgen', 'molebert', 'unimolv1', 'unimolv2', 'chemberta2', 'ecfp', 'rdkitfp']
        for seq in seq_list:
            with open(f'/home/wuke/project/bio_deeplearning/zzz_benchmark/pretrain/saved_models/{seq}.pkl', 'rb') as f:
                seq_embedding = pickle.load(f)
            for smi in smi_list:
                with open(f'/home/wuke/project/bio_deeplearning/zzz_benchmark/pretrain/saved_models/{smi}.pkl', 'rb') as f:
                    smi_embedding = pickle.load(f)
                curr_dataset = []
                for idx, row in data.iterrows():
                    ...
                    # 提取嵌入 (处理可能的KeyError)
                    try:
                        seq_emb = seq_embedding[sequence]
                        smi_emb = smi_embedding[smiles]
                        curr_dataset.append((seq_emb, smi_emb, value))
                    except KeyError as e:
                        print(f"Missing embedding for {e}")
                        continue
                ...
                """Output files."""
                file_MAEs = f'../../Results/output/cv{cv}/MAEs--' + setting + '.txt'
                file_model = f'../../Results/output/cv{cv}/' + setting
                print(file_MAEs)
                print(file_model)
