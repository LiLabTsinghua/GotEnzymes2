...
def main() :
    ...
    # for cv in range(1, 5):
        cv = 4
        Kcat_data = pd.read_csv(f'/home/wuke/project/bio_deeplearning/zzz_benchmark/data/cv/{cv}/EITLEM_KCAT.csv')
        ...
        # np.save('../../Data/input/'+tensor__(compounds), arr)
        # arr = np.asarray(adjacencies, dtype = object)
        # np.save('../../Data/input/'+'adjacencies', arr)
        # arr = np.asarray(regression, dtype = object)
        # np.save('../../Data/input/'+'regression', arr)
        # arr = np.asarray(proteins, dtype = object)
        # np.save('../../Data/input/'+'proteins', arr)
        with open(f'../../Data/input/cv{cv}/'+'compounds'+'.pkl', 'wb') as f:
            pickle.dump(tensor_long(compounds), f)
        with open(f'../../Data/input/cv{cv}/'+'adjacencies'+'.pkl', 'wb') as f:
            pickle.dump(tensor_float(adjacencies), f)
        with open(f'../../Data/input/cv{cv}/'+'regression'+'.pkl', 'wb') as f:
            pickle.dump(tensor_float(regression), f)
        with open(f'../../Data/input/cv{cv}/'+'proteins'+'.pkl', 'wb') as f:
            pickle.dump(tensor_long(proteins), f)
        dump_dictionary(fingerprint_dict, f'../../Data/input/cv{cv}/fingerprint_dict.pickle')
        dump_dictionary(atom_dict, f'../../Data/input/cv{cv}/atom_dict.pickle')
        dump_dictionary(bond_dict, f'../../Data/input/cv{cv}/bond_dict.pickle')
        dump_dictionary(edge_dict, f'../../Data/input/cv{cv}/edge_dict.pickle')
        dump_dictionary(word_dict, f'../../Data/input/cv{cv}/sequence_dict.pickle')
