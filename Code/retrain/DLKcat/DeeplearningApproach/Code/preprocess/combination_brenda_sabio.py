...
with open('../../Data/database/Kcat_brenda_smiles_0820.json', 'r') as infile:
    brenda_name_smiles = json.load(infile)

with open("../../Data/database/Kcat_sabio_clean_unisubstrate.tsv", "r", encoding='utf-8') as file1 :
...
print(len(Kcat_data))  # 49392 #58809 #59647 59808@240820
...
# print(clean_Kcat)
print(len(clean_Kcat))  # 44166 #52477 #
...
print(len(Kcat_data))  # 44166 #52477  57397@240820
...
# print(clean_Kcat)
print(len(clean_Kcat))  # 41558, in which 13454 entries have UniprotID #49226  54652@240820

with open("../../Data/database/Kcat_combination_0731_st.tsv", "w") as outfile :
    records = ['ECNumber', 'EnzymeType', 'Organism', 'Smiles', 'Substrate', 'UniprotID', 'Value', 'Unit']
    outfile.write('\t'.join(records) + '\n')
    for line in clean_Kcat :
        outfile.write('\t'.join(line) + '\n')
'''
这个脚本是用来合并来自BRENDA和Sabio-RK数据库的Kcat数据
补充：两个贼长的带i的代码都不需要跑??
'''