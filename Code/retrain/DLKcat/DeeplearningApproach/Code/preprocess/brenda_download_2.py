...
#             # Save previous ec_table in a EC file:
#             os.chdir(output_path)
#             with open(previous + '.txt', 'w') as fid:
#                 fid.writelines(ec_table)
#             print('Successfully constructed ' + previous + ' file.')
#             os.chdir(input_path)
...
# '''
# 读取所有BRENDA文件名：循环处理每个BRENDA文件：
# 针对每个BRENDA文件，提取EC号码和变量名称。
# 读取BRENDA文件中的所有数据。
# 如果EC号码发生变化，将之前的EC_table保存到相应的EC文件中。
# 重新初始化EC_table。
# 解析每个变量的值：
# 解析BRENDA文件中每个变量（如KM、MW、PATH等）的值，将其添加到EC_table中。
# EC_table的格式为 [variable organism value]，其中包含EC文件中的所有数据。
# 写入最后一个EC文件：
# 在循环结束后，将最后一个EC_table保存到相应的EC文件中。
# 结果是生成包含酶信息的多个EC文件，每个文件对应一个EC号码。文件中包含了酶的不同变量（如KM、MW、PATH等）的信息。
# '''
# # import os
